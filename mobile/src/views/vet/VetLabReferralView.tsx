...
import { animalService, LocalAnimal, formatTagNumber } from '../../services/animalService';
...
export const VetLabReferralView: React.FC = () => {
  const { currentLanguage, t } = useLanguageStore();
  const [requisitions, setRequisitions] = useState<LabRequisition[]>([]);
  const [registeredAnimals, setRegisteredAnimals] = useState<LocalAnimal[]>([]);
  ...
  const [tempLocation, setTempLocation] = useState('Cold Storage Transit Post');
  ...
  const loadData = async () => {
    try {
      const [list, animals] = await Promise.all([
        labService.getRequisitions(),
        animalService.getAllAnimals(),
      ]);
      setRequisitions(list || []);
      setRegisteredAnimals(animals || []);
      if (animals && animals.length > 0) {
        setAnimalTag((prev) => prev || animals[0].tagNumber);
      }
    } catch (err) {
      console.warn('Failed to load lab data or animals:', err);
    }
  };
  ...
  const handleCreateRequisition = async (e: React.FormEvent) => {
    ...
    const matchedAnimal = registeredAnimals.find((a) => a.tagNumber === animalTag);
    const created = await labService.createRequisition({
      ...
      villageName: matchedAnimal?.villageName || 'Field Area',
        ...
        {requisitions.length === 0 ? (
          <div className="p-8 text-center space-y-2 bg-white dark:bg-slate-900 rounded-2xl border border-dashed border-slate-200 dark:border-slate-800">
            ...
            <p className="text-xs font-bold text-slate-700 dark:text-slate-300">
              {currentLanguage === 'en' ? 'No Lab Requisitions Found' : 'कोणतीही प्रयोगशाळा मागणी उपलब्ध नाही'}
            </p>
            <p className="text-[11px] text-slate-500 max-w-xs mx-auto">
              {currentLanguage === 'en'
                ? 'Create a new e-LRF diagnostic referral to track sample cold-chain SLA and PCR confirmations.'
                : 'नमुना तपासणीसाठी नवीन इ-मागणीपत्र नोंदवा.'}
            </p>
          </div>
        ) : (
              ...
              <div>
                ...
                >
                  {registeredAnimals.length === 0 ? (
                    <option value="">{currentLanguage === 'en' ? 'No registered animals found' : 'नोंदणीकृत पशू आढळले नाहीत'}</option>
                  ) : (
                    registeredAnimals.map((a) => (
