...
export const UserProfileView: React.FC<UserProfileViewProps> = ({ onClose }) => {
  const { userProfile, activeRole, logout, setLoginStep } = useAuthStore();
    ...
    <div className="max-w-md mx-auto p-4 space-y-5 animate-in fade-in duration-200 pb-12">
      ...
      <div className="space-y-2 pt-1">
        <div className="flex items-center gap-1.5 text-xs font-bold text-slate-600 dark:text-slate-400 px-1">
          ...
          <span>{currentLanguage === 'en' ? 'Account Role' : 'खाते भूमिका'}</span>
        </div>
        <div className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-3 text-xs text-slate-600 dark:text-slate-400">
          <p className={currentLanguage !== 'en' ? 'lang-devanagari' : ''}>
            {currentLanguage === 'en'
              ? 'Your role is locked to your account credentials. To use a different role, sign out and log in through the correct portal.'
              : 'तुमची भूमिका खात्याशी जोडलेली आहे. वेगळी भूमिका वापरण्यासाठी साइन आउट करून योग्य पोर्टलद्वारे लॉगिन करा.'}
          </p>
        </div>
      </div>
